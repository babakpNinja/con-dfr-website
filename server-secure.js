...
// Generate a secure random session secret (changes on each server restart for extra security)
...
// Initialize database tables with additional security fields
db.exec(`
...
// Create default admin if not exists with secure password
const adminExists = db.prepare('SELECT * FROM admin WHERE username = ?').get('admin');
if (!adminExists) {
  // Generate a more secure default password or use environment variable
...
app.use((req, res, next) => {
  ...
  res.setHeader('Content-Security-Policy', "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: https: http:;");
  next();
});
...
// Log login attempts
function logLoginAttempt(username, ip, userAgent, success) {
  db.prepare(`
...
// Auth middleware with additional checks
const requireAuth = (req, res, next) => {
  if (req.session && req.session.isAdmin && req.session.adminId) {
    // Verify admin still exists
    const admin = db.prepare('SELECT id FROM admin WHERE id = ?').get(req.session.adminId);
    if (admin) {
      return next();
    }
  }
  res.status(401).json({ error: 'Unauthorized', message: 'Please log in again' });
};
...
app.post('/api/login', (req, res) => {
  ...
  // Sanitize username (prevent SQL injection - though we use prepared statements)
  const sanitizedUsername = username.trim().toLowerCase().slice(0, 50);
  
  const admin = db.prepare('SELECT * FROM admin WHERE LOWER(username) = ?').get(sanitizedUsername);
  
  if (admin && bcrypt.compareSync(password, admin.password)) {
    // Successful login
    recordLoginAttempt(ip, true);
    logLoginAttempt(sanitizedUsername, ip, userAgent, true);
    
    // Update last login
    db.prepare('UPDATE admin SET last_login = CURRENT_TIMESTAMP, failed_attempts = 0 WHERE id = ?')
      .run(admin.id);
    ...
    logLoginAttempt(sanitizedUsername, ip, userAgent, false);
    
    if (admin) {
      // Update failed attempts in database
      db.prepare('UPDATE admin SET failed_attempts = failed_attempts + 1 WHERE id = ?')
        .run(admin.id);
    }
...
app.post('/api/admin/change-password', requireAuth, (req, res) => {
  ...
  if (admin && bcrypt.compareSync(currentPassword, admin.password)) {
    ...
    db.prepare('UPDATE admin SET password = ?, password_changed_at = CURRENT_TIMESTAMP WHERE id = ?')
      .run(hashedPassword, req.session.adminId);
...
app.get('/api/partners/type/:type', (req, res) => {
  ...
  const partners = db.prepare('SELECT * FROM partners WHERE partner_type = ? AND is_active = 1 ORDER BY display_order, created_at').all(type);
  res.json(partners);
});
...
app.post('/api/admin/partners', requireAuth, upload.single('image'), (req, res) => {
  ...
  `).run(
    (data.name_en || '').slice(0, 255),
    (data.name_fa || '').slice(0, 255),
    (data.name_tr || '').slice(0, 255),
    (data.name_az || '').slice(0, 255),
    (data.name_ar || '').slice(0, 255),
    (data.name_zh || '').slice(0, 255),
    (data.name_es || '').slice(0, 255),
    (data.description_en || '').slice(0, 2000),
    (data.description_fa || '').slice(0, 2000),
    (data.description_tr || '').slice(0, 2000),
    (data.description_az || '').slice(0, 2000),
    (data.description_ar || '').slice(0, 2000),
    (data.description_zh || '').slice(0, 2000),
    (data.description_es || '').slice(0, 2000),
    imageUrl.slice(0, 500),
    (data.website_url || '').slice(0, 500),
    data.partner_type === 'individual' ? 'individual' : 'organization',
...
app.patch('/api/admin/partners/:id/toggle', requireAuth, (req, res) => {
  ...
  const partner = db.prepare('SELECT is_active FROM partners WHERE id = ?').get(partnerId);
  if (partner) {
    db.prepare('UPDATE partners SET is_active = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
      .run(partner.is_active ? 0 : 1, partnerId);
...
// Admin: Get login logs (for security monitoring)
app.get('/api/admin/login-logs', requireAuth, (req, res) => {
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('   ✓ Rate limiting (5 attempts/15 min)');
  console.log('   ✓ Session security (httpOnly, sameSite)');
  console.log('   ✓ Security headers (XSS, CSRF, Clickjacking)');
